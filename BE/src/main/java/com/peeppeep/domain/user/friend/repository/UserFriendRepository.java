...
public interface UserFriendRepository extends JpaRepository<User, String> {
    ...
    @Query("SELECT uf FROM UserFriend uf WHERE (uf.senderId = :userId OR uf.receiverId = :userId) AND uf.status = :status")
    ...
    Optional<UserFriend> deactivateFriendship(@Param("userId") String userId, @Param("status") String status);
}
